...
const jangSelected_gif_1 = __importDefault(require("../../../assets/photo/jangSelected.gif"));
const karinaSelected_gif_1 = __importDefault(require("../../../assets/photo/karinaSelected.gif"));
const sulSelected_gif_1 = __importDefault(require("../../../assets/photo/sulSelected.gif"));
const yunaSelected_gif_1 = __importDefault(require("../../../assets/photo/yunaSelected.gif"));
const GlobalState_1 = __importDefault(require("../../../JustAnd/GlobalState"));
const CompatiWithJangKaSuYoo = () => {
    const [inputName, setInPutName] = (0, react_1.useState)("");
    const [finalName, setFinalName] = (0, react_1.useState)("");
    const [menubar, setMenubar] = (0, react_1.useState)("");
    const [sex, setSex] = (0, react_1.useState)("");
    const [userSelected, setUserSelected] = (0, react_1.useState)("");
    const [saveData, setSaveData] = (0, react_1.useState)(null);
    const [bringImage, setBringImage] = (0, react_1.useState)("");
    const [isInputSaved, setIsInputSaved] = (0, react_1.useState)(false);
    console.log(saveData, "모든 최종적인 값들입니다.");
    const { collaboResult, setCollaboResult } = (0, GlobalState_1.default)((state) => state.collaboResultData);
    ...
    const renderImage = () => {
        switch (bringImage) {
            case "jang":
                return <img src={jangSelected_gif_1.default} alt="장원영 이미지"/>;
                break;
            case "karina":
                return <img src={karinaSelected_gif_1.default} alt="카리나 이미지"/>;
                break;
            case "sul":
                return <img src={sulSelected_gif_1.default} alt="설윤 이미지"/>;
                break;
            case "yuna":
                return <img src={yunaSelected_gif_1.default} alt="유나 이미지"/>;
            default:
                return null;
        }
    };
    const fetchForCollabo = async () => {
        ...
        try {
            const response = await fetch(`${process.env.REACT_APP_API_URL}/api/collabo`, {
                method: "POST",
                headers: {
    ...
    return (<main className="compatibility-box">
      <h1>장카설유 궁합보기!</h1>
      <div className="sections-container">
        <div className="input-section-wrapper">
          <section className="input-user-section">
            <h2>이름을 입력하세요</h2>
            <input type="text" className="user-name-input" placeholder="이름을 입력하세요" value={inputName} onChange={handleInputChange}/>
            <button onClick={handleInputSave} disabled={isInputSaved}>
              입력
            </button>
            <select className="sex-select" value={sex} onChange={(e) => {
          ...
          <section className="select-target-section">
            <h2>대상을 선택하세요</h2>
            <select className="menubar-select-right" value={userSelected} onChange={(e) => {
            const newMenubar = e.target.value;
...
exports.default = CompatiWithJangKaSuYoo;
...
//# sourceMappingURL=compatiWithJangKaSulYoo.jsx.map