...
const getmyWirte = async (req, res) => {
    ...
    try {
        const userInfo = ORM_1.default.getRepository(userInfoEntity_1.userInfoData);
...
const getMyCommnets = async (req, res) => {
    ...
    const { page, limit, offset } = helperParmas(req.query);
    try {
        const userinfo = ORM_1.default.getRepository(userCommentsEntity_1.userComment);
...
const getMyLikes = async (req, res) => {
    try {
        const { identifier, userEmail, loginType } = req.user;
        const { page, limit, offset } = helperParmas(req.query);
        const userLikes = ORM_1.default.getRepository(userLikeEntity_1.userLike);
        const whereCondition = loginType === "nonSocial"
            ? { userId: identifier }
            : { username: identifier };
